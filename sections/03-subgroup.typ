...
== Subgroup A — Orhan Ağaoğlu <subgroup-a>

=== Tool / Intervention Proposal

I'm proposing something I call the *Community Structural Risk Monitor (CSRM)*. The idea is to give the police a way to do crime prevention that goes beyond just showing up more. Instead of a heatmap that says "crime happens here," CSRM tries to figure out _why_ it's happening and helps the police coordinate with the right people to deal with it: municipalities, health services, social workers, housing corporations. The police still lead the process, but they're doing something smarter with the information than sending more patrols.

How it works: the system watches two things at the same time. One is regular crime reports from the police, stuff that already happened. The other is what I call stress signals, things like emergency calls, social services referrals, housing complaints, kids dropping out of school. These tend to show up before crime gets worse. When both start going up in the same area at the same time, the system opens up a 12-month window and starts pulling in neighbourhood-level data for that area.

The data comes from places like CBS for income statistics, GGD for healthcare access, housing corporations for eviction numbers, and municipal records for things like benefit uptake. None of this is personal data. It's all aggregated at the neighbourhood level. That's a big difference from existing Dutch systems like Top600 in Amsterdam or ProKid 12-SI, which actually create risk scores for individual people @galic2023. I don't think that's the right approach. The system looks at the area as a whole and checks if the conditions there match the crime trends. It never singles anyone out.

At the centre of this design is a strict rule about what data gets in and who gets to see it. Every indicator has to be validated by domain experts (criminologists, sociologists, public health researchers) as something with an established relationship to crime. Nothing gets included just because it's available. And once the data is in, access is tied to responsibility: doctor shortages go to the health ministry, unemployment numbers go to the labour office, housing problems go to housing corporations. If you can't act on it, you can't see it. That principle is what holds the whole system together, and I've been calling it "access follows accountability."
...
I actually expect people to disagree with the flags, and that's fine. Pozzi, Sand, and Jongsma (2026) make the argument for medicine that disagreement between humans and AI isn't a problem to solve, it's actually useful @pozzi2026. Same idea here.

A community worker who's been in the neighbourhood for ten years knows stuff that no dataset is going to capture. Who actually talks to who, what programs are already running, whether things are actually as bad as the numbers make them look. If they look at a flag and go "no, that's not what's happening," they should be the ones to listen to. The pin is basically asking "did you know about this?" not saying "do something about this."

That's the big difference from CAS. The CAS heatmap felt like a fact @waardenburg2018. It had colours and squares and it looked like it knew something. My system is just a dot on a map with a question behind it. It's saying "something was noticed here, do you see it too?"
...
- *GGD / health services* see: healthcare indicators only. How long people wait for a GP, whether there's enough addiction support.
